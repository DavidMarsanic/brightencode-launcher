...
- `internal/browser`, `internal/paths` — app-agnostic; migrating to the shared `brightencode-appkit` package (see below) rather than staying copied verbatim.
...
**Shared scaffolding:** the boilerplate half of the applet shape above (server plumbing, browser/window hosting, job registry, SSE, output/path handling — ~425-455 near-verbatim lines per app) has been extracted into `github.com/DavidMarsanic/brightencode-appkit`, using the same `go get ...@main` composition pattern. `gif-maker`, `archive-manager`, and `audio-transcriber` have migrated (proof-of-pattern pilots); the remaining applets still carry their own copies pending the same migration — see `brightencode-appkit`'s README for the exact steps.
